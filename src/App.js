    ...
    <div className="App">
      <div className="w-screen h-screen flex overflow-hidden">
        ...
        <div className="scrollbar w-full h-screen lg:w-2/4 bg-blue-100 flex flex-col  overflow-y-scroll">
          ...
          {
            selected.id !== 0 && data[selected.id - 1].images.map((item, index) => <div
        ...
        <div className="w-full h-screen scrollbar overflow-y-scroll lg:w-1/4 bg-yellow-100 flex flex-col gap-4 p-4">
          ...
          {(selected.id !== 0 && selected.imageId !== 0) && data[selected.id - 1].images[selected.imageId - 1].links.map((item, index) => <a href={item} target="_blank" className="overflow-hidden bg-sky-200 px-3 py-2 rounded-md shadow-md font-semibold cursor-pointer  hover:scale-105 transition-transform duration-200   ">
            {item}
          </a>)}
        </div>
      </div>
